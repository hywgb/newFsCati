...
// API helpers
func (c *Client) API(cmd string) error {
	return c.sendLine("api " + cmd)
}

func (c *Client) BGAPI(cmd string) error {
	return c.sendLine("bgapi " + cmd)
}

func (c *Client) UUIDKill(uuid, cause string) error {
	if cause == "" { cause = "NORMAL_CLEARING" }
	return c.API(fmt.Sprintf("uuid_kill %s %s", uuid, cause))
}

func (c *Client) UUIDAudioForkStart(uuid, wsURL, params string) error {
	p := params
	if p == "" { p = "{channels=1,sampling=8000,ptime=20,stream-type=sender}" }
